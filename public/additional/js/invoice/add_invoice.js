	...
	$.ajax({
		...
		success: function (data) {
			if (data.status == 'oke') {
				...
				$('#disc_nominal').val(data.invoiceDiscount);
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.status == 'oke') {
					...
					/* $('#gt_harga').val(convertToRupiahNoRp(data.totDebet));
                    $('#gt_pajak').val(convertToRupiahNoRp(data.totPajak));   */                                      
					$('#gt_total').val(convertToRupiahNoRp(data.totKredit - data.totDiscount));                                        
					$('#jumlahData').val(data.jumlahData);                                        
...
function formatRupiah(b, y){
	...
	$('#total_'+b).val(convertToRupiahNoRp(total + pajak_nominal));   
...
function addRow(){	
    ...
    var content = "<table id='myTable' border='1' class='classTable'>"
        ...
        content += "<tfoot>"
        content += "<tr>"        
        content += "<td colspan='2'><b>Sub Total</b></td>"                
        ...
        content += "</tr>"
		content += "<tr>"        
        content += "<td colspan='2'><b>Discount</b></td>"                
        content += "<td colspan='3'><input type='text' class='form-control form-control-sm' style='text-align:right' id='disc_nominal' value='' name='disc_nominal'></td>"        
        content += "<td >"  
		content += "<button type='button' class='btn btn-success px-2 ms-2' id='saveDisc1' onclick='saveDisc()'><i class='bx bxs-save me-0'></i></button>"              
        content += "</td>"                
        content += "</tr>"        
		content += "<tr>"        
        content += "<td colspan='2'><b>TOTAL</b></td>"                
        /* content += "<td ><input type='text' class='form-control form-control-sm' style='text-align:right' id='gt_harga' value='' name='gt_harga' readonly></td>"                
		content += "<td ><input type='text' class='form-control form-control-sm' style='text-align:right' id='gt_pajak' value='' name='gt_pajak' readonly></td>"                 */
        content += "<td colspan='3'><input type='text' class='form-control form-control-sm' style='text-align:right' id='gt_total' value='' name='gt_total' readonly></td>"                
        content += "<td >"  		
        content += "</td>"                
        content += "</tr>"        
        content += "</tfoot>";
...
function saveDisc(){
	...
	var disc = document.getElementById("disc_nominal").value;  	
	var method = "POST";
	var action_url = "" + base_url + "/updateDiscInvoice";  	

	$.ajax({
		type: method,
		url: action_url,
		dataType: 'JSON',
		data: {_token: CSRF_TOKEN, idInvoice:id_invoice, disc:disc},            
		beforeSend: function(){
			BeforeSend();
		},
		complete: function(){
		...
		success: function (data) {    
			...
			if (data.status == 'insert_successful') {                        
				...
				error_noti('Gagal Tambah Data, '+data.msg);                     
				totalDebetKredit();

			} else {
				error_noti('Gagal Tambah Data (Kesalahan Sistem)');
				totalDebetKredit();
			}
		},
