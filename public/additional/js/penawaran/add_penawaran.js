		...
		success: function (data) {
			if (data.status == 'oke') {
				...
				for (b = 0; b < totDetData; b++) {						                        
					...
					content += "<input type='hidden' class='form-control form-control-sm' id='menu_id"+b+"' value='"+(data.data[b].id_menu)+"' name='menu_id[]' readonly>"
					content += "<input type='hidden' class='form-control form-control-sm' id='kode_perkiraan"+b+"' value='"+(data.data[b].kode_perkiraan)+"' name='kode_perkiraan[]' readonly>"
					...
					content += "</td>"
					content += "<td ><input type='text' class='form-control form-control-sm' readonly style='text-align:right' id='harga_"+b+"' value='"+(data.data[b].harga)+"'  name='harga[]'></td>"
    ...
    $( "#deskripsi_"+b ).autocomplete({
        ...
        select: function (event, ui) {           
            if(ui.item.value == "0"){
				...
				var value0 = ui.item.value;
				var splitValue0 = value0.split("-");               
                ...
                $('#id_perkiraan_'+b).val(splitValue0[0]); // save selected id to input
				$('#menu_id'+b).val(splitValue0[1]); // save selected id to input
				$('#harga_'+b).val(splitValue0[2]); // display the selected text
                return false;
            }               
        }
    });
        ...
        if(window.event && window.event.keyCode == 113) {                                
            ...
            $("#myTable > tbody").append(                    
                "<tr class='body' id='row_"+b+"'>"+
                    ...
                    "</td>"+
                    "<td class='text-left'>"+
					"<input type='hidden' class='form-control form-control-sm' id='menu_id"+b+"' value='' name='menu_id[]' readonly>"+
					"<input type='hidden' class='form-control form-control-sm' id='kode_perkiraan"+b+"' value='' name='kode_perkiraan[]' readonly>"+
                    ...
                    "</td>"+
                    "<td ><input type='text' class='form-control form-control-sm' readonly style='text-align:right' id='harga_"+b+"'  value='' name='harga[]'></td>"+
...
function saveDet(b){
	...
	var menuId = document.getElementById("menu_id"+b).value;
    ...
    if(idPenawaran==""){
        ...
        $.ajax({
            ...
            url: action_url,
            dataType: 'JSON',
            data: {_token: CSRF_TOKEN, idJB:idPenawaran, menu_id:menuId, pajak:pajak, id_perkiraan:idPerkiraan, qty:qty, harga:harga, total:total, deskripsi:deskripsi},            
            beforeSend: function(){
                BeforeSend();
            },
            complete: function(){
