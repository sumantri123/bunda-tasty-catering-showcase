...
$(document).ready(function () {
    ...
    function loadData() {
        ...
        data_table  = $('#example2').DataTable({            
            ...
            ajax: {
                "url": "" + base_url + '/getDataJson/supplier',
                'type': 'GET',
                'dataType': 'JSON',
            ...
            columns: [
            {
                ...
                render: function (data, type, row, meta) {
					...
					if(row.id_supplier == null || row.id_supplier == "null" || row.id_supplier == ""){
						result +=
            ...
            "drawCallback": function (settings) {
                $('.btn-edit').on('click', function () {
                    ...
                    $('#nama').val(data.supplier_nama);
					$('#alamat').val(data.supplier_alamat);					
                    $('#telp').val(data.supplier_telp);	
					$('#owner').val(data.supplier_pejabat);				
                ...
                $('.btn-delete').on('click', function () {
                    ...
                    Lobibox.confirm({
                        iconClass: true,
                        title: 'Delete Data',                        
                        msg: 'Yakin Hapus Data "' + data.supplier_nama + '"?',
                        callback: function ($this, type, ev) {
                            if(type=='yes'){
                                deleteProses(data.supplier_id);
                            }        
                        }
                    }); 
    ...
    function deleteProses(id) {
        $.ajax({
            type: 'GET',
            url: "" + base_url + "/delete/supplier/" + id,
            dataType: 'JSON',            
    ...
    function insertUpdateProses() {
        ...
        if (form.valid() == true) {
            ...
            var action_url = "" + base_url + "/supplier";            
            var action_type = "Tambah";
            if (method === "PUT") {
                action_url = "" + base_url + "/supplier/" + $('#id').val();
                action_type = "Ubah";
            }
