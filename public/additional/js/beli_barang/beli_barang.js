...
$(document).ready(function () {
    ...
    function loadData() {
        ...
        data_table  = $('#example2').DataTable({            
			...
			pageLength: 25,		
            ajax: {
                "url": "" + base_url + '/getDataJson/beliBarang',
                'type': 'GET',
                'dataType': 'JSON',
            ...
            "drawCallback": function (settings) {
                $('.btn-edit').on('click', function () {
                    ...
                    $('#id').val(data.m_barang_keluar_id);                    
					$('#nama').val(data.m_barang_keluar_nama);					                    
					$('#kat_barang_keluar').val(data.id_perkiraan);		
                ...
                $('.btn-delete').on('click', function () {
                    ...
                    Lobibox.confirm({
                        iconClass: true,
                        title: 'Delete Data',                        
                        msg: 'Yakin Hapus Data "' + data.m_barang_keluar_nama + '"?',
                        callback: function ($this, type, ev) {
                            if(type=='yes'){
                                deleteProses(data.m_barang_keluar_id);
                            }        
                        }
                    }); 
    ...
    function deleteProses(id) {
        $.ajax({
            type: 'GET',
            url: "" + base_url + "/delete/beliBarang/" + id,
            dataType: 'JSON',            
    ...
    function insertUpdateProses() {
        ...
        if (form.valid() == true) {
            ...
            var action_url = "" + base_url + "/beliBarang";            
            var action_type = "Tambah";
            if (method === "PUT") {
                action_url = "" + base_url + "/beliBarang/" + $('#id').val();
                action_type = "Ubah";
            }
