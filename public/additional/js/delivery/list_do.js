...
$(document).ready(function () {
	...
	$('button#tambah').on('click', function () {
		...
		var urlx = "/addDO/"+kode+"/"+idPenawaran;
		otherPage(urlx)
    ...
    function loadData() {
            ...
            ajax: {
                "url": "" + base_url + '/getDataJson/dokDO',
                'type': 'GET',
                'dataType': 'JSON',
            ...
            "drawCallback": function (settings) {				
				...
				$('.btn-print').on('click', function () {                      
					...
					window.open(base_url+"/cetakDO/"+data.do_id, '_blank', 'left=0,top=0,width=1000,height=700,status=0');					
                ...
                $('.btn-edit').on('click', function () {
                    var data = data_table.row($(this).parents('tr')).data();                    
					var kode = $('#kode').val();
					var urlx = "/editDO/"+kode+"/"+data.do_id+"/"+data.id_penawaran;
				...
				$('.btn-delete').on('click', function () {
                    ...
                    Lobibox.confirm({
						iconClass: true,
						title: 'Delete Data',                        
						msg: 'Yakin Hapus Dokumen"' + data.do_nomor + '"?',
						callback: function ($this, type, ev) {
							if(type=='yes'){
								deleteProses(data.do_id);
							}        
						}
					});                    
                });
