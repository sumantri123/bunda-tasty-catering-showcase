...
function refresh() {
	...
	$.ajax({
		type: 'post',
		url: "" + base_url + "/search/pemesananDet",
		dataType: 'JSON',
		data: {
			_token: CSRF_TOKEN,
			kode: kode,
		...
		success: function (data) {
			if (data.status == 'oke') {
				...
				$('#no_bukti').val(data.pesanNo);
				$('#id_jb').val(data.pesanId);
				$('#tgl').val(data.pesanTgl);				
				$("#supplier").val(data.idSup).trigger('change');                    
                $("#supplier").select2({ width: "100%" });
				$('#perihal').val(data.pesanHal);				
				$('#pajak_global').val(data.pesanPajak);
				...
				for (b = 0; b < totDetData; b++) {						                        
					...
					content += "<td >"
					content += "<input type='text' class='form-control form-control-sm' id='deskripsi_"+b+"' value='"+data.data[b].pesan_deskripsi+"' name='deskripsi[]'>"						
					content += "</td>"
					content += "<td class='text-left'>"
					content += "<input type='hidden' class='form-control form-control-sm' id='barang_keluar_id_"+b+"' value='"+data.data[b].id_barang_keluar+"' name='barang_keluar_id[]' readonly>"
					...
					content += "<input type='number' class='form-control form-control-sm' id='qty_"+b+"' value='"+data.data[b].qty+"' name='qty[]' onkeyup='qtyTotal("+b+", this)'>"                        					
					content += "<input type='hidden' class='form-control form-control-sm' id='jurbag_det_id_"+b+"' value='"+data.data[b].pesan_detail_id+"' name='jurbag_det_id[]' readonly>"
					content += "<input type='hidden' class='form-control form-control-sm' id='method_det_"+b+"' value='PUT' name='method_det[]' readonly>"
					content += "</td>"
					content += "<td ><input type='text' class='form-control form-control-sm' style='text-align:right' id='satuan_"+b+"' value='"+(data.data[b].satuan)+"' name='satuan[]'></td>"
...
function totalDebetKredit(){
    ...
    if(id!=""){
        $.ajax({
            type: 'GET',
            url: "" + base_url + "/total/pemesananDetail/" + id,
            dataType: 'JSON',
            // beforeSend: function () {
...
function autoComplete(b, y){
    ...
    $( "#deskripsi_"+b ).autocomplete({
        source: function( request, response ) {
            // Fetch data
            $.ajax({
            url: "" + base_url + "/getIdBarangKeluar",
            type: 'post',
            dataType: "json",
            data: {
                _token: CSRF_TOKEN,                
        ...
        select: function (event, ui) {           
            if(ui.item.value == "0"){
                ...
                $('#deskripsi_'+b).val(ui.item.label); // m_barang_keluar_nama
                $('#barang_keluar_id_'+b).val(splitValue[1]); 
                $('#id_perkiraan_'+b).val(splitValue[0]); 
                return false;
            }               
        }
    });
...
function addRow(){	
    ...
    var content = "<table id='myTable' border='1' class='classTable'>"
        ...
        content += "<th width='13%' class='text-center'><b>Qty</b></th>"
...
//		content += "<th width='15%' class='text-center'><b>Pajak</b></th>"				
        ...
        content += "<tfoot>"
        content += "<tr>"        
        content += "<td colspan='3'><b>TOTAL</b></td>"                
        ...
        if(window.event && window.event.keyCode == 113) {                                
            ...
            $("#myTable > tbody").append(                    
                "<tr class='body' id='row_"+b+"'>"+
                    ...
                    "</td>"+
                    "<td class='text-left'>"+
					"<input type='hidden' class='form-control form-control-sm' id='barang_keluar_id_"+b+"' value='' name='barang_keluar_id[]' readonly>"+
                    "<input type='hidden' class='form-control form-control-sm' id='id_perkiraan_"+b+"' value='' name='id_perkiraan[]' readonly>"+
                    "<input type='number' class='form-control form-control-sm' id='qty_"+b+"' value='' name='qty[]' onkeyup='qtyTotal("+b+", this)'>"+                    
                    ...
                    "</td>"+
					"<td ><input type='text' class='form-control form-control-sm' style='text-align:right' id='satuan_"+b+"' value='' name='satuan[]'></td>"+
...
$('#formEntry').submit(function(e) {
	...
	var action_url = "" + base_url + "/addPemesanan";
	var action_type = "Tambah";
	if (method === "PUT") {
		action_url = "" + base_url + "/addPemesanan/" + $('#id_jb').val();
		action_type = "Ubah";
	}
		...
		$.ajax({
			...
			success: function (data) {                    
                if (data.status == 'insert_successful') {                        
                    ...
                    $('#id_jb').val(data.id);
					$('#id_jurnal_bagian').val(data.idJurnalBagian);
...
function saveDet(b){
    ...
    var idPemesanan = document.getElementById("id_jb").value;    
	var idJurnalBagian = document.getElementById("id_jurnal_bagian").value;    
    var deskripsi = document.getElementById("deskripsi_"+b).value;                
	var idPemesananDet = document.getElementById("jurbag_det_id_"+b).value;
	...
	var idBarangKeluar = document.getElementById("barang_keluar_id_"+b).value;
    ...
    if(idPemesanan==""){
		...
		var action_url = "" + base_url + "/pemesananDet";  
        var action_type = "Tambah";   
		if (method === "PUT") {			
			action_url = "" + base_url + "/pemesananDet/" + idPemesananDet;
			action_type = "Ubah";
		}
...
function deleteProsesDet(id, b, idJB) {
    $.ajax({
        type: 'GET',
        url: "" + base_url + "/delete/pemesananDetail/" + id+"/" + idJB,
        dataType: 'JSON',
        beforeSend: function(){
            BeforeSend();
        },
