...
$(document).ready(function () {
    ...
    function loadData() {
            ...
            ajax: {
                "url": "" + base_url + '/getDataJson/dokKwNonInvoice',
                'type': 'GET',
                'dataType': 'JSON',
            ...
            "drawCallback": function (settings) {				
				...
				$('.btn-print').on('click', function () {                      
					var data = data_table.row($(this).parents('tr')).data();	
					window.open(base_url+"/cetakKwNonInv/"+data.kw_id, '_blank', 'left=0,top=0,width=1000,height=700,status=0');					
                ...
                $('.btn-edit').on('click', function () {
                    var data = data_table.row($(this).parents('tr')).data();                    
					var kode = $('#kode').val();
					var urlx = "/editKw/"+kode+"/"+data.kw_id;
				...
				$('.btn-delete').on('click', function () {
                    ...
                    Lobibox.confirm({
						iconClass: true,
						title: 'Delete Data',                        
						msg: 'Yakin Hapus Dokumen"' + data.kw_nomor + '"?',
						callback: function ($this, type, ev) {
							if(type=='yes'){
								deleteProses(data.kw_id);
							}        
						}
					});                    
                });
